...
const DRAFT_MARKER = "- This file was generated from the persisted Y-CRM learning draft flow.";
...
function buildSummary(filePath: string): string {
	if (filePath.includes("/customers/")) {
		return "Y-CRM learning draft 升格後的客戶摘要頁";
	}
	if (filePath.includes("/opportunities/")) {
		return "Y-CRM learning draft 升格後的商機摘要頁";
	}
	if (filePath.includes("/operations/")) {
		return "Y-CRM learning draft 升格後的營運摘要頁";
	}
	if (filePath.includes("/playbooks/")) {
		return "Y-CRM learning draft 升格後的 playbook 頁";
	}
	return "Y-CRM learning draft 升格後的知識頁";
}

function appendRowToSectionTable(content: string, section: string, row: string): { content: string; updated: boolean } {
...
function ensureIndexEntries(files: string[], today: string): boolean {
	...
	for (const filePath of files) {
		...
		const row = `| \`${filePath}\` | ${buildSummary(filePath)} | Y-CRM | ${today} |`;
...
function ensureLogEntry(sessionId: string, files: string[], today: string): boolean {
	...
	const header = `## [${today}] promotion | Y-CRM learning draft ${sessionId.slice(0, 8)}`;
...
export function promoteYcrmLearningWikiDrafts(
	sessionId: string,
	draft: YcrmLearningDraft,
	options?: {
