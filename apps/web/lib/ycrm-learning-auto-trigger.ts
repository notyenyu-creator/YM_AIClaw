...
import {
  getSessionMeta,
  updateSessionPlannerLearningDraft,
} from "@/app/api/web-sessions/shared";
import {
  buildYcrmLearningDraft,
  type YcrmLearningDraftMessage,
...
): YcrmLearningDraftMessage[] {
  const result: YcrmLearningDraftMessage[] = [];
  for (
...
export type AutoTriggerSkipReason =
  | "subagent_session"
...
export type AutoTriggerOutcome =
  | { status: "skipped"; reason: AutoTriggerSkipReason }
  | { status: "generated"; sessionId: string };

export function runAutoLearningDraftEvaluation(
  sessionId: string,
  options: { now?: number } = {},
): AutoTriggerOutcome {
  ...
  if (!meta?.plannerPreflight || !meta.plannerContextPack) {
    return { status: "skipped", reason: "no_planner_artifacts" };
  }
  if (!meta.plannerPreflight.shouldRouteToYcrm) {
    return { status: "skipped", reason: "not_routed_to_ycrm" };
  }
  if (!HIGH_VALUE_INTENTS.has(meta.plannerPreflight.intent)) {
    return { status: "skipped", reason: "intent_not_high_value" };
  }

  const existing = meta.plannerLearningDraft;
  ...
  const draft = buildYcrmLearningDraft({
    ...
    planner_preflight: meta.plannerPreflight,
    planner_context_pack: meta.plannerContextPack,
    messages,
  });
  ...
  updateSessionPlannerLearningDraft(sessionId, annotated);
  return { status: "generated", sessionId };
}

export function triggerAutoLearningDraftIfEligible(sessionId: string): void {
  setTimeout(() => {
    try {
      runAutoLearningDraftEvaluation(sessionId);
    } catch (err) {
      console.warn(
        "[ycrm-learning-auto-trigger] Skipped due to unexpected error:",
        err,
      );
    }
  }, 0);
