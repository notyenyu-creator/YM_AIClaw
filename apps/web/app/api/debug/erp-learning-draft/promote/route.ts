...
import { applyErpLearningDraftPromotion } from "@/lib/erp-learning-draft";
import { promoteErpLearningWikiDrafts } from "@/lib/erp-learning-promotion";
...
export async function POST(req: Request) {
  ...
  // Promotion always requires reviewer identity + reason (audit trail).
  if (!reviewerActor) {
    return Response.json(
  ...
  if (!existing) {
    return Response.json(
      { error: "No ERP learning draft persisted for this session." },
      { status: 404 },
    );
  }

  // State machine: normal promotion requires "written" state.
  // Force-override requires the draft to already be in "promotion_conflicted".
  ...
  const result = promoteErpLearningWikiDrafts(body.session_id, existing, {
    forceConflictOverride: forceOverride,
  });

  const updated = applyErpLearningDraftPromotion(existing, {
